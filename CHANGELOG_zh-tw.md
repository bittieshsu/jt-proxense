...
### 新增
- **疑難排解頁新增「解除安裝」分類** (四條)：完整移除、移除但保留設定與使用者、
  解除安裝程式可能印出的兩種訊息，以及手動清掉移除到一半的狀態。
- **`uninstall.sh` 失敗時會印出疑難排解網址**，語言規則與安裝程式相同。它的兩個失敗
  路徑 —— 沒有以 root 執行，以及被管線呼叫而沒有終端機可確認 —— 跟 `install.sh` 裡的
  任何一個一樣，都可能是某個人與這個專案的第一次接觸。輸入不是 `remove` 仍然安靜結束：
  那是使用者刻意中止，不是失敗，在那裡放求助連結只是雜訊。

## [1.1.2] — 2026-09-14

### 新增
- **安裝與升級疑難排解頁**，中英文各一份，附可點選的目錄與即時過濾的搜尋框：
  <https://jasoncheng7115.github.io/jt-proxense/troubleshooting.zh-tw.html>

  二十五條，每一條都對應真實存在的失敗 —— `install.sh` 裡的每一個 `die`、本
  變更記錄中記載過的問題 (PEP 668、未宣告的 `six` 相依)，以及 v1.1.0 刻意改變的
  行為。升級那一節的第一條就是最多人問的：**單純 `git pull` 不算升級**，因為它
  不會重新安裝 systemd unit、不會更新 Python 相依套件，也不會套用擁有者與權限的
  修復。

- **`install.sh` 失敗時會印出那個網址**，並依機器語系選擇讀者的語言：中文系統得到
  中文頁，其餘一律英文 (依序看 `LC_ALL`、`LC_MESSAGES`、`LANG`，也就是 C 函式庫的
  解析順序)。一個沒有指出下一步的錯誤訊息，就是評估停下來的地方。

### 修正
- **這個網站從來沒有載入過自己的字型。** `style.css` 從 landing site 上線那天就
  `@import` `fonts/fonts.css`，而那個檔案從來沒有進過 repo —— 它一直是 404，所以
  每一頁都退回 `system-ui`，而隨附的 `.woff2` 就躺在旁邊沒被使用。更糟的是，五個
  **Rajdhani** 檔案 (整個網站的內文字型) 全都是 Google 的 404 錯誤頁，被存成
  `.woff2` 副檔名：一次無聲失敗的下載，被當成成功提交了進來。

  兩者都從程式自己的 `dist/fonts/` 修復 —— 那裡有正確的副本，所以完全沒有對外下載。
  `tests/test_troubleshooting_page.py` 現在會斷言每一個隨附字型的開頭確實是 `wOF2`
  魔術位元組，以及被 import 的樣式表確實存在。

- **共用樣式表裡的 `section { padding: 80px 0 }`** 套用到了新頁面的分類上，讓每一節
  上下各多出 80px —— 只有一條的分類因此變成 212px 的空洞。以明確的重設處理，而不是
  改掉元素名稱。

### 文件
- 安裝程式自己的檔頭現在有 `JT_PROXENSE_REF` 的說明；它在 v1.1.0 加入時，上方的用法
  區塊沒有一併更新。
- 首頁的導覽列與頁尾都連到疑難排解頁，各自連到自己的語言版本。

## [1.1.1] — 2026-09-13

### 修正
- **設定檔裡只要有這一版不認識的欄位，服務就起不來。** v1.1.0 把
  `load_config()` 改成 fail-closed，對**損壞的**檔案而言這是對的 —— 退回
  `Config()` 等於 `auth.enabled: false` 加上 `host: 0.0.0.0`。但**無法辨識的
  欄位**不是損壞：它是另一個版本寫出來的檔案、手動編輯，或是打錯字。它會以
  非預期的關鍵字參數傳進 dataclass 建構子，例外被新的 fail-closed 路徑接住，
  然後 daemon 帶著一句在講關鍵字參數的訊息拒絕啟動。升級不該讓一行過時的設定
  變成服務中斷。

  現在未知欄位會被逐一具名警告後忽略，周圍的設定 —— 尤其是 `auth.enabled` ——
  完整保留。真正無法解析的 YAML 與錯誤的頂層結構仍然拒絕啟動，那才是
  fail-closed 當初要擋的東西。

  **沒有任何已發佈的設定檔受影響。** `save_config()` 寫出的是當前 dataclass 的
  `asdict()`，所以程式自己寫出來的檔案永遠能被讀回；已對照驗證過：一台線上安裝
  中橫跨 v0.7 到 v1.0 的七個真實設定檔、`config.example.yaml` 的每一個歷史版本，
  以及 `server/config.py` 十個修訂版裡宣告過的每一個欄位 —— 從來沒有任何一個被
  移除過。真正會踩到的是手動編輯過的檔案，而判斷錯誤的代價是 daemon 起不來。

### 文件
- v1.1.0 那一則補上**升級注意事項**:反向代理與 CORS 的變更需要部分操作者在升級
  前先動手，而第一項的後果先前寫得太輕描淡寫。如果你的代理不在同一台主機上、又
  沒有列進 `auth.trusted_proxies`，現在所有用戶端在速率限制上會共用代理的位址
  —— 任何人登入失敗五次，所有人一起被鎖十五分鐘。

## [1.1.0] — 2026-09-09

外部審查 v1.0.1 後，找出一批預設值與邊界不是「偏弱」而是「錯的」。以下每一項
都先對照程式碼確認過才動手，每個修正都附上一個「修好前確實會失敗」的測試。

其中三項升級後行為會改變，各自標註在條目內。

### 升級注意事項

上面有三項會改變既有安裝的行為。檔案權限與擁有者的修復是**重新執行安裝器**時才會套用的；單純 `git pull` 只會留下舊的 systemd unit，什麼都不會修。

- **如果你的反向代理不在同一台主機上**，升級前請先把它加進 `auth.trusted_proxies`。否則每個請求現在都會帶著代理的位址而不是真實用戶端的：稽核記錄會把代理當成每個動作的來源，而且更麻煩的是，每 IP 登入鎖定會把所有使用者算成同一個 IP —— 任何人失敗五次，所有人一起被鎖十五分鐘。真的被鎖了用 `jt-proxense unlock --all` 解除。被忽略的 `X-Forwarded-For` 會對每個來源記錄一次警告，所以 journal 會告訴你正在發生這件事。
- **如果你把 UI 放在不同的來源**，請把該來源設進 `server.cors_origins`。跨來源存取現在預設關閉，填 `*` 會被忽略。
- **其餘不需要改設定。** 你檔案裡既有的設定都會保留，而這一版不認識的欄位會附警告被忽略，不會被當成損壞的檔案 —— 升級不該讓一行過時的設定變成起不來的服務。

### 安全性

- **`config.yaml` 可能是全世界可讀，而裡面放著 PVE API token。** 安裝器用單純的
  `cat >` 建檔，所以權限完全取決於安裝當下 shell 的 umask —— 常見預設會產生
  `0644`，我們在真實主機上就是看到這個值。`save_config()` 同樣沒有設定權限，而
  `config_backups/` 因為 `copy2` 會保留來源權限，跟著一起繼承。安裝器現在以
  `umask 077` 建檔，並在每次執行時修復既有安裝與其備份；程式則在每次寫入時強制
  `0600`。

- **設定檔解析失敗會把驗證關掉。** `load_config()` 捕捉所有例外後退回
  `Config()`，而它的預設值是 `auth.enabled: false` 與 `host: 0.0.0.0`。所以一個
  被截斷或手動改壞的 YAML，會在下次重啟時把一台有驗證的機器變成開放的，而且除了
  journal 裡一行記錄之外沒有任何跡象。現在它會拒絕啟動並指出備份目錄位置。**檔案
  不存在**仍然視為首次安裝，照舊採用預設值。

- **儲存設定既非原子操作，失敗了也不會說。** `open(path, "w")` 在寫入第一個位元組
  之前就先清空檔案，所以寫到一半當掉會留下半個設定檔 —— 而上面那項修正現在會對
  這種檔案拒絕啟動。而且整段被 `except Exception: log; return config` 包住，於是
  `POST /api/config` 會在磁碟寫入失敗的情況下回報成功。現在改為暫存檔 + fsync +
  原子改名，序列化結果會先讀回來驗證才取代原檔，失敗會往上傳到呼叫端。

- **`GET /api/config` 沒有任何角色檢查，而且只遮蔽兩個欄位。** POST 有 admin 把關，
  GET 完全沒有；而 `to_dict()` 是 `asdict(self)`，整棵 dataclass 樹。任何已登入的
  工作階段 —— 包含完全沒有任何角色授權的 —— 都讀得到 `server.influx_token`、
  `auth.ldap` 的 bind 密碼，以及每一台節點的位址。現在秘密一律對所有人以哨符取代，
  非 admin 只會拿到 SPA 實際會讀的投影：顯示偏好、警示門檻、主控台模式，以及每個
  叢集的 id、名稱與「是否已設定密碼」。

- **CORS 設了萬用字元來源，同時開啟 `allow_credentials`。** `aiohttp_cors` 無法在
  帶憑證時送出字面的 `*`，於是它改為回送請求端的 Origin —— 等於操作者造訪過的任何
  網站，都能對每一個端點發出帶 cookie 的請求，方法與標頭不限。**行為變更：** 跨來源
  存取現在預設關閉。SPA 由同一個行程提供，本來就不需要授權。若要把 UI 放在別的來源，
  請在 `server.cors_origins` 明確列出；填 `*` 會被忽略。

- **同網段的任何一台都能偽造自己的來源位址。** `X-Forwarded-For` 過去對所有
  RFC1918、link-local 與 loopback 來源一律採信。那個值正是每 IP 登入鎖定所計算的
  身分，也是寫進每一筆稽核記錄的 `source_ip`，所以只要每次嘗試換一個標頭值就能繞過
  鎖定，也能把稽核軌跡指向別人。**行為變更：** 現在只有 loopback 是隱含信任，其他
  位置的反向代理必須寫進 `auth.trusted_proxies`。被忽略的 `X-Forwarded-For` 會對
  每個來源記錄一次警告，否則每一筆稽核記錄的意義都會悄悄改變。

- **服務帳號可以改寫自己正在執行的程式碼。** unit 一邊啟用
  `ProtectSystem=strict`，一邊把整個 `/opt/jt-proxense` 列進 `ReadWritePaths`，
  安裝器又把整棵樹 chown 給服務帳號 —— 所以只要在 web 行程內取得命令執行，就能改
  `run.py` 然後等一次重啟。**行為變更：** 程式碼改為 root 所有、對服務唯讀；
  `ReadWritePaths` 現在只列出 `config.yaml`、`config_backups/`、服務的 `.ssh`
  目錄、`/var/lib/jt-proxense` 與 `/etc/jt-proxense`。`.ssh` 這個例外不是裝飾：
  服務帳號的家目錄**就是**安裝目錄，而那把金鑰是 ZFS、主機升級、開機鏡像與儲存
  下載用來連到 PVE 節點的憑據。

- **每叢集 RBAC 在入口處根本沒有生效。** 授權一直都是
  `(user, cluster_id|*) -> role`，`auth.role_for()` 也解析得完全正確，但
  `role_required()` 讀的是 `role_global`，而那字面上就是 `role_for(user, "*")`。
  於是一個只被授予 `cluster1 operator` 的使用者 rank 是 0，在他明明被授權的那個
  叢集上被每一個裝飾過的端點拒絕；而 VM 層級的處理器 —— 它們呼叫
  `_check_vm_role()`，確實有把叢集傳進去 —— 卻是正確的。兩套授權系統，只有一套對。
  現在 `/api/clusters/{cluster_id}/…` 底下的端點會解析呼叫者在該叢集的角色；沒有
  帶叢集的路由仍然只對 `*` 解析，所以叢集管理員不會因此變成使用者管理員。

- **WebSocket 把整片基礎設施交給任何已登入的工作階段。** `/ws` 只繼承了 middleware
  的「你登入了嗎」，此外沒有任何檢查，接著就送出 `get_all_data()` —— 每個叢集、
  節點、客體與儲存。現在初始快照與廣播都會過濾成該工作階段可見的叢集，並且**每個
  不同範圍只序列化一次**，所以同一個叢集的 N 個檢視者仍然只花一次序列化成本。

### 修正

- **`type: esxi` 從來就選不到 ESXi 轉接器。** `Config.from_dict()` 建立
  `ClusterConfig` 時沒有傳入 `type`，所以不管檔案怎麼寫，這個欄位都會變回
  `"pve"` —— 而且重新儲存設定會把它改寫掉。

- **`npm run lint` 從來沒能執行。** 那個 script 呼叫 ESLint，但 ESLint 不在
  `devDependencies` 裡，而且從來沒有過。改成 `npm run typecheck`
  (`tsc --noEmit`)，它今天就是乾淨的，而且正是它原本要代表的那道檢查。真正的
  ESLint 設定仍未完成。

- **THIRD-PARTY-NOTICES.md 宣稱 Apache 2.0 全文收錄在 LICENSE 裡**，但 LICENSE
  從 v1.0.0 起放的是 AGPL。`test_licensing.py` 本來就是為了防止這件事而寫的，卻
  兩重漏掉：那個檔案不在它檢查的文件清單內，而且它的正則也匹配不到實際用的寫法
  (`Apache License, Version 2.0` —— 逗號和 `Version` 夾在它要找的兩半之間)。
  兩處都已修正。

- **SECURITY.md 還停留在 v0.1／v0.2。** 支援版本、設定檔的實際權限、上面的 CORS
  與 X-Forwarded-For 立場，以及 `verify_ssl`／SSH host key 的取捨，現在都據實
  陳述，包含這個產品**不會**替你防守的部分。

### 測試

- **OWASP 掃描器的 CORS 檢查對這個 codebase 完全是瞎的。** 它比對的是
  `allow_origins=["*"]` (Starlette 的寫法)，而本專案用的是
  `aiohttp_cors.setup(app, defaults={"*": …})`。所以一個帶憑證的萬用字元設定，在
  三十幾個版本裡每一次都拿到綠色勾勾。一個對它看不見的東西回答「乾淨」的檢查器，
  比沒有檢查器更糟。

- **有兩個測試在 Python 3.12 上會永遠卡住。** 從 3.12 起
  `asyncio.Server.wait_closed()` 會真的等待處理器連線關閉；兩個 TCP 稽核轉送測試
  的處理器返回時都沒有關閉 writer，於是整個套件停在第 37 個測試，在目前的直譯器上
  永遠跑不完。

- **有一個測試會開啟操作者的真實資料庫。** `test_cli_db_path.py` 會
  `pop` 掉 `$JTPROXENSE_DB_PATH`，好讓另外兩個測試驗證 config／env 的優先序，結果
  第三個就落到預設值 —— `/var/lib/jt-proxense/jt-proxense.db`，在任何有這個檔案的
  主機上都是營運中的資料庫。

- **CI 從來沒有建置前端**，所以 TypeScript 錯誤可以進到 `main`，只有在手動跑發行
  建置時才會浮現。workflow 現在會做型別檢查、建置，並稽核正式相依套件。它也會在
  **3.10 與 3.12** 兩個版本上跑 pytest：這個 job 過去只跑 3.10，而程式是對著 3.12
  開發的，這正是兩者行為差異在兩個方向都看不見的原因。測試工具鏈已鎖版 —— 不鎖的話，
  一個新的 `pytest-asyncio` 主版本落地，就能讓三個已經打標的版本在沒有改動任何一行
  程式的情況下同時變紅，而執行歷史顯示的正是這件事。

- `scripts/run-tests.sh` 會把暫存檔留在專案內、並降低 CPU 優先權來跑測試；
  `release-gate.sh` 與 `security-full.sh` 現在會解析直譯器 (先 `.venv`，再系統)，
  不再寫死 `python3` —— 而 build host 上的系統 python3 既沒有 pytest 也沒有
  aiohttp。

### 變更

- **安裝器現在安裝最新的發行標籤，而不是 `main` 的頂端。** 追著分支跑，等於兩個
  操作者相隔一天執行同一行安裝指令會拿到不同的程式，而且沒有任何東西可以回滾。
  `JT_PROXENSE_REF` 可覆寫；在還沒發佈任何 tag 之前會回落到分支。

## [1.0.2] — 2026-09-02

### 修正
- **批次主機升級完全沒有 HA guest 的概念。**它自行遷移每一台執行中的 guest,
  包含 HA 管理的在內 —— 這等於跟 HA manager 對打。對 HA guest 下 `vm_migrate`
  會被轉給 `ha-manager migrate`，而 PVE 會**先接受**一個被 STRICT node-affinity
  規則禁止的遷移，再以 exit 2 非同步失敗。結果是節點搬到一半、協調器正確地拒絕
  重開機，然後由操作者手動把散落的 guest 搬回去。

  HA guest 現在改走 Proxmox 為這個情境設計的機制：
  `ha-manager crm-command node-maintenance enable <node>`。HA manager 會自行把
  自己管理的服務搬走、記住每一台原本在哪，並在解除維護時搬回來；LRM 的 watchdog
  會維持啟用直到全部搬完。我們會等節點真的清空 (CRM 指令是排入佇列的，它的離開
  狀態碼並不代表有東西搬動)，沒清空就不重開機。事後若無法解除維護模式，記錄會
  寫出解除用的完整指令 —— 因為被靜靜留在維護模式的節點，從此不會再承載任何 HA
  服務。

- **協調器是唯一一條沒有詢問 `migrate_guard` 就搬動 guest 的路徑。**單台遷移與
  節點維護早就接上了它，偏偏這條無人值守清空整台主機的流程沒有。現在會在動任何
  guest **之前**，依 HA 親和性規則與儲存可用性過濾每台 guest 的候選目標，讓不可
  能成立的遷移以「拒絕」呈現，而不是搬到一半才發現。規劃器也會區分「沒有目標放
  得下」與「沒有目標被允許」—— 這兩者要修的東西不同。

- **在搬動工作負載之前，什麼都沒檢查。**新增的前置檢查會驗證 SSH 可達性、根檔案
  系統餘裕 (低於 2 GiB 拒絕、低於 5 GiB 警告 —— dist-upgrade 把 `/` 塞爆會讓
  dpkg 停在半設定狀態)，以及 apt 套件庫是否可達，並記錄待更新套件數。無法升級的
  主機現在會在 guest 還待在原位時就被發現。

  `preflight` 狀態刻意設計為可重跑：它是唯一一個「什麼都還沒被改動」的非終端
  狀態，因此守護程序在此時重啟，只需重跑該主機，不必標記為失敗等人工檢視。

- **節點上有 HA guest 時，原地模式會被拒絕。**原地模式會關閉 guest，而 HA manager
  會把任何 `request_state: started` 的服務立刻再啟動 —— 在一台即將重開機的節點上
  形成無解的迴圈。

- **「讀不到 HA 狀態」被當成「沒有 HA」回報。**上述修改的第一版在
  `/cluster/ha/status/current` 讀取失敗時回傳空清單 —— 於是一個缺少 `Sys.Audit`
  的 API token、或短暫的連線失誤，就會告訴協調器「這台節點沒有 HA guest」，讓它
  直接退回手動遷移。也就是這項修改本身要防的那個 bug，而且變成靜默發生。現在
  「未知」是獨立的答案並會拒絕該主機，排空等待也絕不會把「讀不到」當成「已排空」。

- **長達 30 分鐘的 HA 排空等待無法中斷。**協調器裡其他每一個長等待都會檢查中止
  旗標，只有這個沒有。

- **同一個叢集可以同時跑兩個升級批次。**每個 job 各自維護 in-flight 集合且看不見
  對方，於是兩者會各自清空一台節點，而規劃時用的是對方也正在花用的記憶體池。現在
  啟動第二個批次會回應 `409 cluster_busy`。
...
### 修正
- **兩個唯讀端點的權限被設得高於 `viewer`，導致非管理員看到空白頁面。**
  `GET /pools` 與 `GET /backup-jobs` 各自被**兩個模組**註冊；aiohttp 只採用先註冊的
  那個，而兩次勝出的都要求比落敗的唯讀模組更高的角色。viewer 可以讀取單一集區，卻
  無法列出集區清單。`tests/test_route_uniqueness.py` 現在會讓任何重複路由導致建置
  失敗，並檢查每個唯讀路徑實際解析到的角色。
- **Ceph 集區表格顯示的內容全都不是真的。** `CephAdminModal` 是照 PVE 的欄位名撰寫，
  但後端會先把 PVE 映射成 `CephPool` dataclass，而該類別一個都沒帶。名稱欄顯示
  「 (pool undefined) 」、每一列共用 React key `undefined`，銷毀對話框則要求操作者確認
  銷毀名為「undefined」的集區。該請求最終送往 `.../ceph/pool/` (名稱為空) 並回傳
  404，因此資料從未真正面臨風險。`CephPool` 現在帶有 `pool_id` 、 `min_size` 、
  `pool_type` 、 `application` 與 `crush_rule` 。
- **在 PVE 9 上，銷毀 Ceph 集區從來就不可能成功。** 用戶端使用複數形
  `/nodes/{n}/ceph/pool**s**/{name}`;實際的 9.2 節點只列出 `pool`，對 `pools` 回應
  「no such resource」。現在優先嘗試單數形，並保留對舊版的退回機制。
- **開機碟更換路徑使用了已知會誤讀的 `lsblk` 解析方式。**
  `zfs_admin._boot_layout` 使用 `-r`，其 raw 模式會塌縮空欄位，因此沒有 PARTTYPE 但
  有 FSTYPE 的分割區會把檔案系統移入類型欄位 —— 而這個函式決定哪個分割區要被格式化
  為 vfat 。`boot_mirror.py` 早已針對此修正，`zfs_admin.py` 中的複本則沒有。現在只有
  一份實作 `parse_boot_layout`，並且以詢問 `lsblk` 的方式排除非分割區的列，而非依據
  結尾數字判斷 (`nvme0n1` 是一顆磁碟，而其名稱結尾正是數字)。
- **格式錯誤的請求內容會回傳 500 與 traceback 。** 五個處理器直接呼叫
  `await request.json()` 而未保護；現在會回應 `400 {"error": "bad_json"}` 。
- **遷移疊層可能在客體仍在移動時就消失。** 遷移完成的 10 秒備援計時器從未被取消，而
  該 effect 又依賴自身狀態，因此同一客體先前遷移留下的計時器會刪除當前遷移的項目 ——
  疊層在移動途中消失，看起來就像遷移失敗。
- **客體標籤未經正規化就使用。** PVE 在某些回應中以分號字串傳送 `tags`，在其他回應中
  則是陣列，而型別宣告為 `string[]`;對字串執行 `.map()` 會逐字元迭代，`tags[0]` 會回傳
  第一個字母，因此標記為「alpha」的 VM 會被歸類到「a」。
- **共用儲存在警示列表與命令面板中被逐節點重複列出**，因此五節點叢集上的單一 PBS 會
  產生五筆相同警示與五筆搜尋結果。
- **60 個 keyframe 的動畫收在 `translateY(0)` / `blur(0)` 而非 `none` 。** 殘留的
  transform 會讓該元素成為所有 `position: fixed` 子孫元素的容器塊 —— 正是右鍵選單
  出現在距離游標一個側邊欄寬處的成因。現在有參數化測試逐一檢查每個 keyframe 。
- 兩個裸 `alert()` 在 useDialogs 遷移中存活下來，因為當時的檢查 grep 的是
  `window.alert(`，而這兩處沒有前綴。
- 主控台密碼提示對所有叢集都顯示 `root@pam` —— 對於設定為其他 PVE 帳號的叢集，這是
  錯誤的指示。叢集摘要現在會帶上真實的使用者名稱。

### 變更
- 集區端點由 `pools_view` 負責；`backup_jobs` 只保留共用的正規化函式。任一模組都不再
  註冊另一個模組已宣告的路由。

## [0.9.6] — 2026-08-05

### 修正
- **概觀的「基礎架構」卡片顯示 0，但頁首同時寫著 8/8 。** 這是 0.9.4 引入的迴歸：當
  數字跳動被延後到頁面進場動畫之後時，「已播放過」旗標是在計時器**排程時**就被清掉，
  而清理函式只取消了 rAF，從未取消尚未觸發的計時器。資料幾乎總是在該延遲期間抵達，
  於是 effect 重新執行並走了直接賦值的路徑，原本的計時器隨後仍然觸發，並帶著閉包中
  捕捉到的舊值，把顯示值一路動畫回零。該卡片上的每一個數字都受影響。
- **側欄收合會在每一影格重排整個應用程式。** 容器對 `grid-template-columns` 做過渡，
  而那是版面動畫：每一影格都要重新排版所有概觀卡片、儀表與畫布。在實際頁面上以每次
  切換 900 毫秒量測：開啟時只有 8 個影格，關閉後為 24 個，long task 時間減半。現在
  寬度會一次到位 (只重排一次)，運動感改由合成圖層負責。
- **顯示器越寬，概觀看起來越空。** `.cluster-grid` 在 1200px 以上被固定為三欄，因此
  1900px 螢幕上的四個叢集會排成 3 + 1，第二列有三分之二空白。單純提高欄數是錯的解法
  —— 在 2560px 下，六條固定軌道對應四張卡片，反而讓右側三分之一留白。兩個網格現在都
  改用 `repeat(auto-fit, minmax(300px, 1fr))`，未被佔用的軌道會自動收合。

### 變更
- **側欄收合/展開的動態編排** —— 一道掃描線自面板頂端掃下、右側邊緣充能、展開時導覽
  標籤逐項解碼上線，以及貫穿兩個方向的圖示脈衝。全部為一次性：驅動用的 class 會在結束
  後移除，靜止時完全沒有動畫。所有 keyframe 均收在 `transform: none` —— 殘留的
  transform 會使側欄成為 `position: fixed` 子孫元素的容器塊。並遵循
...
### 新增
- **節點設定檔下載** (`node_config_backup.py`，節點右鍵選單 → 設定備份，僅管理員)。
  兩種收集模式：
  - **API** (預設) — 僅透過 PVE REST 建構壓縮檔，因此在從未安裝過 SSH 金鑰的節點
    上也能使用。每個端點一份 JSON (儲存、存取/ACL 、防火牆、HA 、 SDN 、資源集區、
    通知，以及節點的網路/DNS/時間/apt/憑證/磁碟)，外加 `/etc/hosts` 原文與所有客體
    設定。壓縮檔內的 `MANIFEST.txt` 會逐項列出 API **讀不到**的項目
    (`/etc/fstab` 、 `/etc/default/grub` 、 `/etc/chrony` 等)，避免被誤認為可還原的
    `/etc` 複本。
  - **SSH** — `tar czf -` 串流至 stdout，不在節點寫入任何檔案，取得逐位元組的原始檔。
  - 機密檔案 (`/etc/pve/priv` 、 `/etc/ceph` 、 `/etc/ssh` 、 `*.key` 、
    `shadow.cfg`)預設排除，需明確勾選，並記錄為獨立的稽核動作；對話框會說明叢集 CA
    私鑰可用於偽造任何使用者的登入憑證。
  - PVE 自身的系統報告 (`pvereport`)需勾選才納入，並警告其中嵌有客體**備註**欄位，
    該欄位經常含有明文帳密。
  - 選填密碼加密，採用 OpenSSL `enc` 容器格式 (AES-256-CBC + PBKDF2)，可用標準
    `openssl` 解開，不需要 jt-proxense 。密碼走 POST body，絕不進入 URL 。
- **ZFS 磁碟清冊新增 SMART 健康狀況欄位** — PVE 本來就會回傳 `health` 與 `wearout`,
  只是表格從未顯示。排序時將異常磁碟排在最前。

### 修正
- **節點右鍵選單出現在遠離游標處。** `ClusterCore` 的選單未經 portal，導致
  `position: fixed` 相對於 `.view-container` 解析，而其 `page-enter` 動畫會留下
  transform —— 所有座標都被右移一個側邊欄的寬度。另外選單有十一項時，在畫面中段點擊
  上下都放不下，舊邏輯會將它跳至視窗頂端；現改為貼著游標並內部捲動。
- **備份排程的「下次執行」永遠是空的。** PVE 的欄位名為 `next-run`,JS 無法以
  `j.next_run` 讀取含連字號的鍵。修正原本落在 `backup_jobs.py`，但該處其實是**死碼**
  —— `pdm_backups.py` 註冊了同一條路由且優先生效。現在兩者共用同一份正規化函式。
  停用中的排程也不再顯示下次執行時間。
- **主機升級的失敗徽章難以辨識** — 白字配實心 `#ff0040` 與 8px 外光暈，是全站唯一
  如此繪製的徽章。現已改為與其它狀態一致的配色。
- **熱度圖聲稱「近 30 次取樣」**，但那 30 格其實裝著同一個讀數的複本，且從未說明 30 次
  取樣是多久 (各叢集為 2 秒或 10 秒)。現改為回報實測的時間跨距，並在累積中時明確標示。

### 安全性
- 除 `SshUnavailable` 以外的 SSH 錯誤 (特別是 asyncssh 的 `PermissionDenied`) 會以
  500 加 traceback 內容逸出；現統一回傳 JSON，並引導至 SSH 設定說明。
- 以測試釘住：API 模式的端點清單不得包含任何含 `priv` 或 `ssh` 的路徑。

## [0.9.3] — 2026-08-02

### 新增
- **作業結果可跨重啟續追** (migration 011,`pending_tasks`) —— 追蹤 PVE 作業
  真實結果的 watcher 原本只存在記憶體中，一次部署就會把進行中的丟掉，結果列
  再也不會出現 (和「追蹤器壞掉」完全無法區分，我為此繞了不少路)。現在進行中
  的作業會持久化並於啟動時接續；已超過觀察窗的則記為 `timeout` 而非默默遺忘。
  該表的內容正好等於「還欠的結果」。
- **建立 / 還原 / 磁碟放置前置檢查** (`create_guard.py`) —— 另外三件 PVE 會
  接受、然後在作業裡失敗的事，每一件都可能留下半成品客體：
  - **VMID 已被使用。** PVE 的 id 是叢集範圍，「這個節點上沒有」不等於可用。
    還原時另外警告 `force` 會摧毀目前佔用該 id 的客體。
  - **目標節點沒有該橋接。** 對照節點真實的介面清單；實體網卡不會被當成橋接。
  - **儲存不接受該內容類型。** 沿用 0.9.2 的 content 白名單，VM 磁碟不能放到
    只支援 ISO 的儲存，磁碟搬移也不能指向那裡。
  已對真實叢集驗證：用 VMID 147、用 `vmbr99`、用 `iso,vztmpl` 儲存建立，
  三者都被擋下並附上原因與可行的替代選項。

### 修正
- 備份、還原、複製的追蹤原本套用 15 分鐘的預設觀察窗。實際一次到 PBS 的
  vzdump 超過該時間，結果被記為逾時，而它其實還在正常進行；這些操作改為 6 小時。
...
### 新增
- **遷移前置檢查** (`migrate_guard.py`、`ha_affinity.py`) —— PVE 會接受它做不到
  的遷移，幾分鐘後才在非同步作業裡失敗，而那時介面早已回報成功。現在送出前
  會檢查兩項前置條件：
  - **HA 節點親和性。** *strict* 的節點親和性規則可能禁止客體在目標節點執行，
    `ha-manager` 會以 exit 2 失敗。同時支援 PVE 8 (`/cluster/ha/groups` 加
    `restricted`) 與 PVE 9 (`/cluster/ha/rules` 的 `node-affinity`)，並處理
    「PVE 9 的 groups 端點會回傳錯誤而非空陣列」這個陷阱。
  - **儲存可用性。** 比表面上細微：`local-zfs` 是 `shared=false` 卻在每個節點
    都存在，所以「目標有沒有這個儲存名稱」會回答有，而資料其實在來源節點。
    儲存在目標不存在、或線上遷移需要複製非共享磁碟卻未指定時，都會擋下。
  兩者都以 409 回應，並指出是哪條規則、哪些儲存、哪些節點才合法。
  `GET .../migrate-targets` 提供同樣的判斷，讓選單只列出真正到得了的節點。
  維護撤離改為逐一客體挑選目標，並回報被略過的客體。
- **備份儲存前置檢查** —— 儲存的 content 白名單只有在 vzdump 執行時才由 PVE
  強制。備份到只支援 `images,rootdir` 的儲存現在會事先擋下，並列出可用的儲存。
- **作業結果追蹤** (`task_outcome.py`) —— PVE 對長時間作業立即回傳 UPID，而
  處理程式過去在那一刻就寫下 `result="ok"`。因此一個失敗的快照刪除，在稽核
  記錄裡永遠是成功。現在送出記為 `submitted`，追蹤器在作業結束後**追加**一筆
  帶真實 `exitstatus` 的記錄。已套用於快照、客體刪除、複製、備份、還原、Ceph
  與網路套用等 22 個破壞性操作。逾時記為 `timeout`、節點連不上記為 `unknown`
  —— **絕不記為 `error`**，因為回報一個沒有觀察到的失敗，是另一種謊言。

### 修正
- **遷移前置檢查原本跑在授權之前。** 它的 409 內容會列出 HA 規則、允許節點與
  儲存配置 —— 等於一張叢集地圖 —— 任何已登入使用者都能從拒絕訊息讀到。現在
  授權優先，並加上回歸測試釘住兩個遷移處理程式的順序。
- 服務重啟後輪詢快取尚未填滿時，`migrate-targets` 會回答「沒有目標」，讀起來
  像是「這個客體不能遷移」。現在在盤點完成前回報 `unknown`。
- 節點名稱誤讀成 `NodeMetrics.name`(實際欄位是 `node`)，導致每個客體的目標
  清單都靜默為空。

### 安全性
- 所有遷移與備份前置檢查端點都在既有的角色控管之後；破壞性遷移仍需原有的
  目標確認。拒絕訊息只有通過授權後才可能取得。
...
### 新增
- **根集區開機鏡像** (`boot_mirror.py` + `BootMirrorWizard`，管理員) ——
  更換 `rpool` 的磁碟是本產品風險最高的操作：這個集區同時承載開機載入程式、
  節點設定，以及透過 `local-zfs` 存放的客體磁碟。原廠工具只給你一頁 wiki 和一個
  shell，這裡改成分段、逐項檢查、可續看的流程：
  - **三種情境，用偵測而不是假設。** `add_mirror`(單碟→鏡像)、`replace_live` (健康碟的預防性更換)、`replace_dead`(FAULTED / REMOVED / UNAVAIL)。
    第三種最重要，也是手寫流程最常漏掉的：換碟通常**就是因為碟壞了**，而壞掉的
    碟根本讀不出分割表。這時會改從存活成員複製，並使用 `zpool replace` 而非
    `zpool attach`。若所有成員都已故障，則明確告知無救援路徑。
  - **伺服器端前置檢查，不過就擋。** 根在 ZFS、目標就是根集區、沒有正在進行的
    清理或重建、有健康的複製來源、新碟是空的、不在集區內、且容量不小於來源 (整張 GPT 都要複製，所以這項**不能**強制略過)。磁區大小不一致與開機韌體
    列為提醒。
  - **備份閘門。** 以 `/cluster/backup-info/not-backed-up` 檢查該節點的客體。
    有客體沒備份就**阻擋**，只能由操作者明確勾選承擔風險才放行，且該承認會寫入
    稽核記錄。備份資訊讀不到時**同樣阻擋** —— 在這種閘門上誤報「安全」，
    比沒有閘門更危險。
  - **離開頁面回來看得到進度。** 重建動輒數小時，因此作業狀態、階段與完整指令
    時間軸都存在資料庫 (migration 010)。關掉分頁、換一台電腦再回來，百分比
    仍在前進；ZFS 頁面上的橫幅可直接點回任何進行中的作業。服務重啟時會重新
    接上監看，而不是把作業標成孤兒 —— 因為重建本身從未停止。
  - **卸除舊碟永遠不會自動執行。** 只有在伺服器對**當下的集區**重新確認重建
    已完成後才會提供，並在卸除後自動執行 `proxmox-boot-tool clean` 與
    `refresh`，避免日後每次核心更新都對已不存在的 ESP 報警。
  - 每道指令都原文記錄，並且一道一次 SSH 連線分開執行，失敗時能精確指出是哪一步；
    第一次寫入之前會先用 `sgdisk --backup=` 備份 GPT。
- `docs/sop-rpool-mirror.md` —— 本功能所實作的操作程序。
- `docs/sop-demo-video.md` —— 示範影片製作的完整流程 (錄影、字幕、配樂、
  授權義務、驗收)。

### 修正
- **模態視窗內的下拉選單無法點選。** `CyberSelect` 的選項清單以 portal 呈現、
  `z-index` 為 2000，但模態遮罩是 4000–9999，導致清單被畫在模態**後面**:
  DOM 裡存在、完全不透明、卻點不到。已提高到 12050，並加上回歸測試，
  日後若有任何遮罩蓋過它就會測試失敗。
- **`lsblk -r` 欄位錯位。** raw 模式會把空欄位收合，因此「沒有 PARTTYPE 但有
  FSTYPE」的分割區 (實機上出現的 `rbd1  ext4`) 會把檔案系統誤讀成分割區類型 ——
  那會挑錯 ESP。開機鏡像的分割用途偵測改用 `lsblk -P` 的 key=value 輸出。

### 安全性
- 開機鏡像的所有路由皆限管理員；裝置名稱一律正規化為 `/dev/disk/by-id/`
  (拒絕 `/dev/sdX`、by-path 與路徑穿越);破壞性的啟動需要重新輸入集區名稱確認；
  前置檢查、啟動、卸除、中止全部寫入稽核記錄，包含備份閘門是否被誰承認略過。
...
- ZFS 讀取因權限不足或節點無法連線而失敗時，現在回報 `zfs_read_failed` (403/502)，而非呈現成「此節點沒有 ZFS」。
...
- **回歸缺陷 (v0.8.12):每次叢集作業輪詢都回傳 400。** `/cluster/tasks` 端點
  並不接受 `limit` 查詢參數 — v0.8.12 傳入此參數導致 PVE 拒絕每一次請求 (「property is not defined in schema」)，使作業檢視與矩陣作業覆蓋層全空。
...
- **作業：叢集作業清單默默被截到約 50 筆。** `get_cluster_tasks` 現在把 `limit`
  傳給 PVE，用舊 vmid/使用者篩選才會有結果。`stop_task` 的 UPID 補上 URL 編碼 (與讀取路徑一致);移除重複的 `get_task_status`;時間戳/位移解析加防護。
- **HA 管理 guest 的遷移連線。** 矩陣現在能正確解析 `hamigrate` 任務的來源節點 (原本抓成 CRM 節點)，所以 HA 遷移會畫出來源→目的連線、失敗提示也標對來源。
  遷移稽核補記 `bwlimit`。
...
### 變更
- **主機升級的疏散改為強制檢查絕對記憶體餘量。** 先前疏散只用相對負載 (CPU%+MEM%)來平衡目標，可能把某台目標的 RAM 超額配置而 OOM —— 最糟情況是無
  swap / HA 目標自我 fence。現在會檢查每個目標的**可用位元組**、保留每台餘量 (max 4 GiB 或 5%)、絕不把裝不下的 guest 塞進去，且若目標池整體容不下就**中止該
...
- 無伺服器面變更——僅前端 canvas 繪製。v0.8.8 的 OWASP ZAP baseline (0 High / 0 Medium)與手動滲透測試持續涵蓋本版；ZAP baseline 重跑確認
  0 High / 0 Medium。
...
- **重大 —— 修正未經身分驗證的任意檔案讀取 (路徑穿越)。** 靜態資源處理器
  ...
  回傳 daemon 讀得到的任何檔案 —— **完全免驗證**。已證實可外洩 `config.yaml` (PVE API token)、Fernet `master.key`(可解密整個 secret store)、SQLite
...
- **DAST 檢測納入發佈流程。** 每個版本現在都要跑 OWASP ZAP baseline 掃描 (用完即丟的本機目標，絕不掃正式站)，須回報 **0 High / 0 Medium** 才能發佈
...
- **跨源隔離：** 加入 `Cross-Origin-Opener-Policy`、`Cross-Origin-Resource-Policy` (皆 same-origin) 與 `Cross-Origin-Embedder-Policy: require-corp`(ZAP 的
...
- **同叢集遷移連線畫不出來。** 矩陣方格中，即時遷移本應從來源畫一條發光
  連線到目標節點上的 incoming「ghost」格。原本 `incomingMirror` 判斷 (只該用於跨叢集鏡像遷移) 在同叢集遷移時會誤判 VM 自己的 ghost (因為
...
- **登入頁語言切換。** 伺服器渲染的登入頁右上角加入可見的 `EN / 中文`
  ...
  網址也沒有 `?lang=`，登入頁仍會沿用該語言。頁面的中文改用內附的 Plix (IBM Plex Sans CJK，OFL-1.1) 科幻字體 — 針對登入頁字符子集化至約
...
- **VM 匯出 OVA / Hyper-V VHDX** (operator 以上，僅 QEMU)。VM 右鍵選單
  ...
  版本；缺 `ovftool` 等相依會明示)、從節點的檔案型儲存挑選暫存目錄 (以預估來源磁碟總量驗證可用空間與可寫性；空間吃緊需明確確認)、
...
- vm_export：所有路由 operator 以上；格式／ESXi 版本／節點名白名單
  驗證；所有遠端值經 `shlex.quote`；下載檔名須在作業記錄的輸出清單內 (無法路徑跳脫)；清除路徑重新驗證伺服器產生的前綴；工具安裝、作業
  建立／刪除與下載皆稽核。
...
- **設定匯出包改用每次隨機 salt** (PBKDF2 200k);本版之前的包仍可匯入 (保留舊固定 salt 路徑)。密鑰庫 master key 權限設定失敗改為大聲記錄。
...
- **`uninstall.sh` —— 一行完整移除。**
  ...
  `/opt/jt-proxense`、`/var/lib/jt-proxense`、`/etc/jt-proxense` (master key)、以及服務使用者。透過控制終端確認 (可在 pipe 下運作)
  或加 `--yes`。
...
- **每輪輪詢的日誌噪音 + 重複 Ceph 探測造成 idle CPU 偏高。**
  ...
  這些 per-poll 訊息改為 DEBUG，且偵測到沒有 Ceph 的叢集會退避 (改成每 60 輪才重查一次，而非每輪)。在 4 叢集測試機上，idle
...
- **節點畫面找回即時心電圖。**「波形範圍」選擇器在 RRD 時間窗 (1 小時 / 24 小時 / 7 天 / 30 天 / 1 年) 之外新增 **即時** 選項。
  選取後，每張節點卡會顯示由即時 WebSocket 指標餵入的動態心電圖 (CPU / MEM / IOW) —— 即 v0.5.3 之前的行為；其餘時間窗仍維持歷史
...
- **不疏散 (in-place)主機升級模式。** 主機升級精靈的「VM 疏散
  ...
  客體線上遷移到其他節點，而是把該主機上執行中的客體正常關機 (ACPI)，主機跑 `apt dist-upgrade` 並重開機，完成後再把原本在跑
...
- **跨頁面樣式一致性整理**：所有 view 改用標準 panel-card 容器樣式
  ...
  節點卡與節點詳細彈窗、`HoloMatrix` 縮圖卡、`HealthMonitor` 告警卡 (保留事故等級色左側 3px 軌道為刻意例外)、`CephConstellation`
...
- **對話框按鈕加 leading SVG icons**：VMCloneModal / VMMigrateModal /
  VMDeleteModal (取消 × / 複製 / 遷移箭頭 / 垃圾桶) 以及 VMConfigModal (編輯鉛筆 / 取消 × / 套用打勾)，按鈕本身改用 inline-flex 對齊。
